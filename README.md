# OdeHash v5 — Rust Implementation

A cryptographic hash function based on **coupled Riccati ordinary differential equations** over the prime field F_(2²⁵⁵ − 19).
...
```bash
cargo run --release -- -f path/to/file
```

```bash
cargo run --release -- -f path/to/file -p
```

```bash
cargo run --release -- -f path/to/file --format
```
...
```bash
cargo test --release --test test_v5
```
...
```bash
cargo run --release --example check_v5
```
...
This implementation produces **bit-for-bit identical** hashes to the C
implementation for all test vectors.
